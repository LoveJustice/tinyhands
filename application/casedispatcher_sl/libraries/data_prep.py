...
def extract_role_series(role_series):
    """
    Cleans and processes a Pandas Series of role entries.
    Each cell can contain multiple roles separated by delimiters.

    Returns:
        processed_series: A Series where each value is a sorted list of unique, cleaned roles.
        roles_txt_series: A Series of roles joined into a semicolon-separated string.
        unique_elements: A set of all unique roles found.
    """
    ...
    def clean_item(item):
        # Strip whitespace
        item = item.strip()
        # Define replacements (all keys are lower-case because we lower the input)
        replacements = {
            "rapisuspect": "suspect",
            "hosuspect": "suspect",
            "diver": "driver",
        ...
        item = re.sub(r"\.$", "", item)
        # Return the replacement if available,
        # return default_value if the item is "none",
        # otherwise return the original item.
        return replacements.get(item, default_value if item == "none" else item)

    def process_series(series):
        # Fill missing values and replace empty strings
        series = series.fillna(default_value).replace("", default_value)
        # Lower-case and standardize delimiters
        series = series.str.lower().replace(
            {r"\[": "", ", ": ";", "/": ";", " ;": ";"}, regex=True
        )
        # Split roles on delimiter and clean each role
...
def create_role_dummies(role_series, top_n=5):
    """
    Given a Pandas Series of role entries, this function:
      1. Processes the series to clean and split the roles.
      2. Identifies the top_n most frequent roles.
      3. Creates dummy variables for these top roles (with column names prefixed by 'role_').
      4. Combines any remaining roles (per row) into a semicolon-separated string stored in 'role_other'.

    The original 'role' column can then be dropped, and the returned DataFrame
    contains only the new dummy columns.

    Args:
        role_series (pd.Series): Original role column from your DataFrame.
        top_n (int): Number of most frequent roles to breakout as dummy variables.

    Returns:
        pd.DataFrame: DataFrame with dummy columns (role_<value>) and a 'role_other' column.
    """
    # Process the role series using the extraction function.
    processed_series, _, _ = extract_role_series(role_series)

    # Count the frequency of each role (excluding the default placeholder)
    role_counter = Counter(
        role for roles in processed_series
        for role in roles if role != "missing information"
    )

    # Get the top_n roles (most common roles)
    top_roles = [role for role, _ in role_counter.most_common(top_n)]

    # Prepare a new DataFrame for the dummy variables
    dummy_df = pd.DataFrame(index=processed_series.index)

    # Create a dummy column for each top role.
    for role in top_roles:
        dummy_df[f"role_{role}"] = processed_series.apply(lambda lst: 1 if role in lst else 0)

    # For each row, join all roles not in top_roles (and not the default value) into role_other.
    def get_other_roles(lst):
        others = sorted({role for role in lst if role not in top_roles and role != "missing information"})
        return ";".join(others) if others else ""

    dummy_df["role_other"] = processed_series.apply(get_other_roles)

    # Replace any empty strings with the default text.
    dummy_df["role_other"] = dummy_df["role_other"].replace("", "missing information")
