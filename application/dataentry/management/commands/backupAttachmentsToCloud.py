...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for relative_path in self.explore_storage(media_existing_storage_backend, path_prefix):
            if path_prefix:
                if path_prefix not in relative_path:
                    logger.debug(f'Skipping {relative_path} because it does not contain {path_prefix}')
                    continue
            relative_path_without_prefix = relative_path[len(path_prefix)+1:]
            logger.debug('Checking backup of ' + relative_path_without_prefix)
            ...
            if date:
                ...
                if mtime_date < date:
                    logger.debug(f'Skipping {relative_path_without_prefix} because ${mtime_date} is older than {date}')
                    continue
            is_on_azure = media_backups_storage_backend.exists(relative_path_without_prefix)
            bytes_on_file_system = file_on_file_system.read()
            if is_on_azure:
                file_on_azure: File = media_backups_storage_backend.open(relative_path_without_prefix)
                ...
                if hash_of_azure != hash_of_file_system:
                    logger.debug(f'Hash of {relative_path_without_prefix} backup changed, replacing on Azure')
                    media_backups_storage_backend.delete(relative_path_without_prefix)
                    media_backups_storage_backend.save(relative_path_without_prefix, file_on_file_system)
                # else it is there and is the same, no action
            else:
                logger.debug(f'{relative_path_without_prefix} backup not in azure, adding to Azure')
                # Are you getting
    ...
    def explore_storage(self, media_storage_backend, path: str = 'media') -> str:
