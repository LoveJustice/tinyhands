...
from dataentry.models import Person, PersonFormCache, SiteSettings


class Command(BaseCommand):
    def handle(self, *args, **options):
        existing_cached_ids = PersonFormCache.objects.all().values_list('person__id', flat=True)
        uncached_persons = Person.objects.all().exclude(id__in=existing_cached_ids)
        
        for person in uncached_persons:
            PersonFormCache.load_cache(person)
        ...
        try:
            recent_count = site_settings.get_setting_value_by_name('PersonFormCache_recent')
        except:
            recent_count = 100
        try:
            aged_count = site_settings.get_setting_value_by_name('PersonFormCache_aged')
        except:
            aged_count = 3000
        ...
        aged_cache_records = PersonFormCache.objects.all().order_by('creation_time', 'id')
        last_id = -1
        processed_count = 0
        for aged_cache_record in aged_cache_records:
            if aged_cache_record.person.id == last_id:
                continue
            PersonFormCache.load_cache(aged_cache_record.person)
