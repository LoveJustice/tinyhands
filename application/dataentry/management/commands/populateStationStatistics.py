...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        suspects = LegalCaseSuspect.objects.filter(verdict_submitted_date__gte=start_date, verdict_submitted_date__lt=end_date, verdict='Conviction')
        for suspect in suspects:
            entry = StationStatistics.objects.get(year_month=year_month, station=suspect.legal_case.station)
            if entry.convictions is None:
                entry.convictions = 1
            else:
                entry.convictions += 1
            entry.save()
