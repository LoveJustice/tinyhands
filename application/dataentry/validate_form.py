...
from .models.form import FormCategory, FormValidation, FormValidationLevel, FormValidationQuestion, Question, \
    QuestionLayout, QuestionStorage
...
class ValidateForm:
    ...
    def add_error_or_warning(self, category_name, category_index, validation, data_string=''):
        ...
        if category_index is not None:
            msg = category_name + ' ' + str(
                category_index) + ': ' + validation.error_warning_message + ' ' + data_string
        else:
    ...
    def should_do_validation(self, form_data, validation):
        ...
        try:
            if validation.params is not None and 'starting_submission_date' in validation.params:
                ...
                questions = QuestionStorage.objects.filter(field_name='logbook_submitted').values_list('question',
                                                                                                       flat=True)
    ...
    def at_least_one_true(self, form_data, validation, validation_questions, category_index, general):
        for validation_question in validation_questions:
            ...
            if answer is not None and (
                    isinstance(answer, bool) and answer == True or isinstance(answer, str) and answer.strip() != ''):
    ...
    def regex_match(self, form_data, validation, questions, category_index, general):
        ...
        for validation_question in questions:
            question = validation_question.question
            answer = form_data.get_answer(question)
            result = re.match(regex, str(answer))
            if result is None:
        ...
        for validation_question in validation_questions:
            ...
            if answer is not None and answer != '':
                if isinstance(answer, date):
                    ...
                    test_datetime = datetime(answer.year, answer.month, answer.day, answer.hour, answer.minute,
                                             answer.second)
                else:
                    continue
    ...
    def custom_interceptee_phone_verified(self, form_data, validation, questions, category_index, general):
        if validation.params is not None and 'category_name' in validation.params:
            category_name = validation.params['category_name']
            category_id = self.find_category_id_by_name(form_data, category_name)
            missing = False
            if category_id in form_data.card_dict:
                for card in form_data.card_dict[category_id]:
                    if card.form_object.person is not None:
                        person = card.form_object.person
                        if person.phone_contact is not None and person.phone_contact != '' and not person.phone_verified:
                            self.add_error_or_warning(category_name, category_index, validation)
                            break
        else:
            category_name = ''
            ...
            tmp_validation = FormValidation()
            tmp_validation.level = validation.level
            tmp_validation.error_warning_message = (
                    'Incorrect configuration for validation:' + validation.error_warning_message)
            self.add_error_or_warning(category_name, None, tmp_validation)
    ...
    def card_reference(self, form_data, validation, validation_questions, category_index, general):
        ...
        if (getattr(base_form, 'card_dict',
                    None) is None or validation.params is None or 'category_name' not in validation.params or
    ...
    def __init__(self, form, form_data, ignore_warnings, mode='update'):
        self.validations = {
            ...
            'interceptee_phone_verified': self.custom_interceptee_phone_verified,
        }
        ...
        if getattr(self.form_data.form_object, 'status',
                   None) is not None and self.form_data.form_object.status != 'in-progress':
                ...
                for validation_question in validation_questions:
                    if validation_question.question.id in question_to_validation_set:
                        ...
                        if set_key is None:
                            ...
                            self.errors.append(
                                'Validation mixes main form and card questions. Validation id=' + str(validation.id) +
    ...
    def perform_validation(self, validation, form_data, category_index=None):
        if validation.level.name in self.validations_to_perform:
            if validation.trigger is not None:
                ...
                if validation.params is not None and 'trigger_part' in validation.params:
                    trigger_value = self.get_answer_part(trigger_value, validation, 'trigger_part')
            ...
            if should_validate:
                ...
                for question in questions:
                    ...
                    elif question.question.id in self.question_map and the_category != self.question_map[
    ...
    def validate(self):
        ...
        if self.main_form in self.validation_set:
            for validation in self.validation_set[self.main_form]:
                if validation.validation_type.name in self.validations:
                    ...
                    logger.error(
                        "validation #" + validation.id + " specifies an unimplemented validation:" + validation.validation_type.name)
                    self.response_code = status.HTTP_500_INTERNAL_SERVER_ERROR
        ...
        for category_id, card_list in self.form_data.card_dict.items():
            ...
            for card in card_list:
                category_count += 1
                category_label = None
                if getattr(card.form_object, 'validate_label', None) is not None:
                    category_label = card.form_object.validate_label()
                if category_label is None:
                    category_label = category_count

                self.check_not_null(category_id, card, category_index=category_label)
                self.check_invalid_date(category_id, card, category_index=category_label)

                if category_id in self.validation_set:
                    for validation in self.validation_set[category_id]:
                        if validation.validation_type.name in self.validations:
                            if self.should_do_validation(self.form_data, validation):
                                self.perform_validation(validation, card, category_index=category_label)
                        else:
                            logger.error("validation #" + str(
                                validation.id) + " specifies an unimplemented validation:" + validation.validation_type.name)
                            self.response_code = status.HTTP_500_INTERNAL_SERVER_ERROR
