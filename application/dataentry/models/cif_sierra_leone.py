...
class PotentialVictimSierraLeone(PotentialVictimCore):
    cif = models.ForeignKey(CifSierraLeone)
    
    def set_parent(self, the_parent):
        self.cif = the_parent

class TransporationSierraLeone(TransporationCore):
    cif = models.ForeignKey(CifSierraLeone)
    
    def set_parent(self, the_parent):
        self.cif = the_parent

class PersonBoxSierraLeone(PersonBoxCore):
    cif = models.ForeignKey(CifSierraLeone)
...
class LocationBoxSierraLeone(LocationBoxCore):
    cif = models.ForeignKey(CifSierraLeone)
    
    def set_parent(self, the_parent):
        self.cif = the_parent

class VehicleBoxSierraLeone(VehicleBoxCore):
    cif = models.ForeignKey(CifSierraLeone)
    
    def set_parent(self, the_parent):
        self.cif = the_parent

class CifAttachmentSierraLeone(CifAttachment):
    cif = models.ForeignKey(CifSierraLeone)
