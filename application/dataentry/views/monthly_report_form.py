...
class MonthlyReportFormViewSet(BaseFormViewSet):
    ...
    def summary(self, request, country_id, year, month):
        ...
        reports = MonthlyReport.objects.filter(year=year, month=month, station__in=stations).order_by(
        ...
        section_totals = {}
        report_count = 0
        stations_without_reports = list(stations_that_should_have_reports)
        for report in reports:
            report_count += 1
            report_data = [report.station.station_code]
            # This station has a report, so remove it from the list
            stations_without_reports = [station for station in stations_without_reports if
                                        station.station_code != report.station.station_code]
            total = 0
            count = 0
            for field_name in report_sections:
        ...
        station_codes_without_reports = []
        for station in stations_without_reports:
            station_codes_without_reports.append(station.station_code)
        results["stations_without_reports"] = station_codes_without_reports

        percentage_of_stations_with_reports = 1 - (float(len(stations_without_reports)) / float(len(stations)))
        results["num_stations_without_reports"] = len(stations_without_reports)
        results["total_stations_that_should_have_reports"] = len(stations_that_should_have_reports)
        results["percentage_of_stations_with_reports"] = percentage_of_stations_with_reports
